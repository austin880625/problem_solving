...
void build2D(Node2D *&O,int L,int R)
{
    if(!O)O=new Node2D();
    if(L==R){O->minv=O->maxv=grid[X][L];return ;}
    int mid=(L+R)/2;
    build2D(O->lc,L,mid);
    build2D(O->rc,mid+1,R);
    O->minv=min(getMin(O->lc),getMin(O->rc));
    O->maxv=max(getMax(O->lc),getMax(O->rc));
}
...
void build1D(Node1D *&O,int L,int R)
{
    if(!O)O=new Node1D();
    X=-1;
    if(L==R){X=L;build2D(O->T,0,N-1);return ;}
    if(L<R)
    {
        int mid=(L+R)/2;
        build1D(O->lc,L,mid);
...
void change2D(Node2D *&O,int L,int R)
{
    if(L==R){grid[X][L]=CV;O->minv=O->maxv=CV;return ;}
    int mid=(L+R)/2;
    if(CY<=mid)change2D(O->lc,L,mid);
    else change2D(O->rc,mid+1,R);
    O->minv=min(O->lc->minv,O->rc->maxv);
    O->maxv=max(O->lc->maxv,O->rc->maxv);
}

void change1D(Node1D *&O,int L,int R)
{
    X=-1;
    if(L==R){X=L;change2D(O->T,0,N-1);return ;}
    int mid=(L+R)/2;
