...
void insert(int o,int xpyl,int xpyr,int xmyl,int xmyr,bool c)
{
    ...
    while(xpyl!=xpyr||xmyl!=xmyr)
    {
        ...
        {
            mid=(xpyl+xpyr)/2;
            if(IX+IY<=mid)now=ch[now][0] ? ch[now][0] : ch[now][0]=newnode(),xpyr=mid,c=!c;
            else now=ch[now][1] ? ch[now][1] : ch[now][1]=newnode(),xpyl=mid+1,c=!c;
        }
        else
        {
            mid=(xmyl+xmyr)/2;
            if(xmyl+xmyr<0)mid--;
            if(IX-IY<=mid)now=ch[now][0] ? ch[now][0] : ch[now][0]=newnode(),xmyr=mid,c=!c;
            else now=ch[now][1] ? ch[now][1] : ch[now][1]=newnode(),xmyl=mid+1,c=!c;
        }
    }
    //cout<<o<<" "<<xpyl<<" "<<xpyr<<" "<<xmyl<<" "<<xmyr<<endl;
    sz[now]++;
